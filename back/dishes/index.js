    ...
    if(req.payload && req.payload.name) {
        if(req.params.dish_id) {
            ...
            Dish.findById(req.params.dish_id).exec((err, dish) => {
                if(req.payload.restau_id !== dish.restau_id.toString()) return res.status(403).json({"message": "You don't have permission to modify this"});
        ...
        } else res.status(404).json({"message": "Not found dish"});
...
router.delete('/:dish_id', auth, function(req, res) {
    ...
    } else res.status(404).json({"message": "No dish"})
})
